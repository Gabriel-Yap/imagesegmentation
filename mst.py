...
def image_to_graph(I):
    ...
    for i in range(height):
        for j in range(width):
            vertices_2d[i,j] = G.insert_vertex(I[i,j], i, j)

    for i in range(height):
        for j in range(width):
            current_vertex = vertices_2d[i, j]

            if j+1 < width:
                right_vertex = vertices_2d[i, j+1]
                weight = compute_weight(current_vertex._color, right_vertex._color)
                G.insert_edge(current_vertex, right_vertex, weight)
...
def segment_kruskal(I, num_seg):
    ...
    vertex_to_position = {}
    segmentation = np.empty( I.shape ) #matrix
    ...
    for i in range(height):
        for j in range(width):
           for vertex in G.vertices():
                if vertex._i == i and vertex._j == j:
